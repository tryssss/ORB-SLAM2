...
//C#:初始化类内变量
    ...
    sum_track_all(0),num_lost(0),sum_re_f(0)//C#***********************************************************************************************
{
...
void Tracking::Track()
{
    ...
    // mState为tracking的状态机(#C统计mState为LOST的次数？)
    ...
    else// 步骤2：跟踪
    {
        ...
        // C#:在这里统计跟踪的总次数sum_track_all***********************************************************************************************
        // C#:RecentlyFrame,暂定20张，并在Track()中更新****************************************************************************************
        // C#:RecentlyFrame,考虑是否可以替代KeyFrame，则要考虑KeyFrame的作用*******************************************************************
        ++sum_track_all;
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // 运动模型是空的或刚完成重定位
                // mCurrentFrame.mnId<mnLastRelocFrameId+2这个判断不应该有（#C:？？）
            ...
            {
                // BOW搜索，PnP求解位姿
                // C#:在这统计lost的次数，使用全局变量num_lost,应当在.h文件中定义，并且在构造函数中初始化*****************************************
                // C#:这里是进行traking和局部地图的统计次数*************************************************************************************
                ++num_lost;
                bOK = Relocalization();
            }
        }
        ...
        if(bOK)
            mState = OK;
        else/*C#:不行就去重定位，可是再次失败呢？再重定位？这里的设置机制不好*******************************************************************/
            mState=LOST;
        ...
        // C#:以上注释错误，仅仅是Reset if the camera get lost soon after initialization********************************************************
        if(mState==LOST)
        {
    ...
    else
    {
        ...
        // 如果跟踪失败，则相对位姿使用上一次值
        // C#:这里的跟踪失败应该算是重定位失败，那么在这里设置变量sum_re_f，统计次数***************************************************************
        ++sum_re_f;
...
{
    // 步骤1：如果用户在界面上选择local模式，那么将不插入关键帧
    ...
    // 步骤2：判断是否距离上一次插入关键帧的时间太短(C#:??好像不对)
    ...
    const float ratioMap = (float)nMap/(float)(std::max(1,nTotal));//C#:避免0/0的出现
...
bool Tracking::Relocalization()
{
    ...
    // 步骤2：找到与当前帧相似的候选关键帧
    // C#:这里应当在KeyFrameDatabase类DetectRelocalizationCandidates函数中修改，并且在KeyFrameDatabase类内创造public的RecentlyFrame**************
    // C#:RecentlyFrame,暂定20张，并在Track()中更新********************************************************************************************
    // C#:在KeyFrameDatabase类内创造Detect函数还是另外构建一个类？如果直接使用KFD，都不用创造ReF，还是重新创建一个类吧******************************
    vector<KeyFrame*> vpCandidateKFs = mpKeyFrameDB->DetectRelocalizationCandidates(&mCurrentFrame);//这里要修改为Frame类型
    //vpCandidateKF候选关键帧
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        if(pKF->isBad())//isBad在Frame里没有，在RFD内将Frame转换成KF，然后后面再在Tracking将没有选上的删除。**************************************
            vbDiscarded[i] = true;
        else
        {
            // 步骤3：通过BoW进行匹配
